...
public class JdbcOrderRepository implements OrderRepository {
   ...
   /* private static final String TOTAL_ORDERS="select count(*) from `order` where customer_account = ";
    ...
    // 获取收入
    private static final String TOTAL_INCOME = "SELECT COALESCE(SUM(v.per_income), 0) AS income FROM (SELECT order_id, create_time, order_price AS per_income FROM `order` WHERE order_state=2 AND delivery_state=2) AS v";
    /*private static final String TOTAL_ORDERS="select count(*) from `order` where customer_account = ";
    private static  final String CUSTOMER_ORDERS="select * from `order` where customer_account = ";*/
